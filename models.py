...
class Venue(db.Model):
    ...
    def upcoming_shows(self):
        upcoming_shows = db.session.query(Show).join(Venue).filter(Show.venue_id == self.id).filter(Show.start_time > datetime.now()).all()
...
class Artist(db.Model):
    ...
    def upcoming_shows(self):
        upcoming_shows = db.session.query(Show).join(Artist).filter(Show.artist_id == self.id).filter(Show.start_time > datetime.now()).all()
        return  upcoming_shows
